...
function StatusNode({ data }: NodeProps<Node<StatusNodeData>>) {
  return (
    <div className={`px-4 py-2 rounded-lg border-2 shadow-sm ${statusColors[data.status]}`}>
...
function EditableNode({ id, data, selected }: NodeProps<Node<EditableNodeData>>) {
...
function CustomPathEdge({ sourceX, sourceY, targetX, targetY }: EdgeProps) {
...
function Flow() {
  ...
  return (
    <ReactFlow nodes={nodes} edges={edges} edgeTypes={edgeTypes} onEdgesChange={onEdgesChange} />
  )
}
```
...
- [Node Resizer](https://reactflow.dev/api-reference/components/node-res
